...
export default class Claim {
    ...
    get = async () => {
        return await claimModel.find({type: CLAIM_TYPE_ENUM.MAIN.code}).populate("company").populate("job").lean().exec();
    };

    getByID = async (id) => {
        const claim = await claimModel.findOne({ _id: id }).populate("company").populate("job").lean().exec();
    ...
    getByNumber = async (number) => {
        return await claimModel.findOne({ number: number }).populate("company").populate("job").lean().exec();
    }

    getAmpsByParent = async (parentId) => {
        return await claimModel.find({ parentClaim: parentId, type: CLAIM_TYPE_ENUM.AMP.code })
    }

    getManyByCompany = async (company) => {
        return await claimModel.findOne({ company: company }).populate("company").populate("job").lean().exec();
    }
